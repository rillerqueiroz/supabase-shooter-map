...
import { CheckCircle2, XCircle, Loader2, Ban, AlertCircle } from "lucide-react";

const WEBHOOK_URL = "https://n8n.superavit.app.br/webhook/cancela-boletos-asaas";
...
interface BulkCancelamentoModalProps {
  open: boolean;
...
export function BulkCancelamentoModal({
  open,
  onOpenChange,
  cobrancas,
  onComplete,
}: BulkCancelamentoModalProps) {
  const [isProcessing, setIsProcessing] = useState(false);
  const [statuses, setStatuses] = useState<CancelStatus[]>([]);
    ...
    try {
      const payload = {
        cobrancas: [{
          ...
          nome: cobranca.nome,
          credor_cedrus: cobranca.credor_cedrus,
        }],
      };
      ...
      setStatuses((prev) =>
        prev.map((s) =>
          s.id === cobranca.Identificador
            ? { ...s, status: success ? "success" : "error", errorMessage: success ? undefined : "Falha no cancelamento" }
            : s
        )
      );
  ...
  const getStatusIcon = (status: CancelStatus["status"]) => {
    switch (status) {
              ...
              {statuses.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground truncate">{item.nome || "Sem nome"} • {formatCurrency(item.valor)}</div>
                  </div>
                  {item.status === "success" && <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">Cancelado</Badge>}
          ...
          {isComplete && (
            <div className={`flex items-center gap-2 p-3 rounded-lg ${errorCount > 0 ? "bg-yellow-50 text-yellow-800" : "bg-green-50 text-green-800"}`}>
              ...
              <span className="font-medium">
                {errorCount > 0 ? `Concluído com ${errorCount} erro(s).` : "Todas as cobranças foram canceladas com sucesso!"}
              </span>
            </div>
          )}
        </div>
        ...
        <DialogFooter className="gap-2">
          {!isProcessing && !isComplete && (
            <>
              ...
              <Button variant="destructive" onClick={() => { abortRef.current = false; processQueue(); }}>
                <Ban className="h-4 w-4 mr-2" />
                Iniciar Cancelamento ({cobrancas.length})
              </Button>
            </>
          )}
