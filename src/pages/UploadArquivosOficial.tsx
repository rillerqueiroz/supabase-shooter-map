  ...
  if (vencStr) {
    ...
    if (vencEfetivo >= today) {
      ...
      if (todayDayOfWeek === 1) {
        ...
        lastSaturday.setDate(today.getDate() - 2); // sábado anterior
        lastSaturday.setHours(0, 0, 0, 0);
        const lastSunday = new Date(today);
        lastSunday.setDate(today.getDate() - 1); // domingo anterior
      ...
      for (let i = 0; i < updateRecords.length; i += batchSize) {
        ...
        for (const r of batchSlice) {
          ...
          if (Object.keys(updates).length > 0) {
            const { error } = await supabase.from("base_tudobelo_intermediaria").update(updates).eq("id", r.id);
            if (error) {
        ...
        {analysis.peopleAnalysis && (
          <Card className="border-l-4 border-l-blue-500">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Pessoas existentes não serão atualizadas. Novas pessoas são criadas apenas para títulos novos no momento do envio.
              </p>
            </CardContent>
          </Card>
        )}
