...
export default function UploadArquivos() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      try {
        console.log('[upload-teste] analisando pessoas em', result.records.length, 'registros');
        result.peopleAnalysis = await analyzePeopleForRecords(result.records, { externalSystem: 'tudobelo' });
        console.log('[upload-teste] peopleAnalysis:', result.peopleAnalysis);
      } catch (e: any) {
        console.warn('[upload-teste] falha ao analisar pessoas:', e?.message, e);
